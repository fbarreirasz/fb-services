...
export default function CalculadoraPage() {
  ...
  function getMultiplier(rate: number, stam: boolean, store: boolean) {
    const s = stam ? 1.5 : 1.0;
    const base = rate * s;
    return store ? base * 1.5 : base;
  }
  ...
  const multLabel = (() => {
    ...
    if (hasStam) l += ' × 1.5';
    if (hasStore) l += ' × 1.5';
    return l + ` = ${getMultiplier(currentRate, hasStam, hasStore).toFixed(2)}x`;
  })();
  ...
  function calcular() {
    ...
    if (!levelFrom || !levelTo || levelFrom >= levelTo) {
      setError('Level desejado deve ser maior que o atual.');
      return;
    }
    if (!rawKK || rawKK <= 0) {
      setError('Informe o raw XP/h.');
      return;
    }
    if (levelFrom < 1 || levelTo > 2000) {
      setError('Level entre 1 e 2000.');
      return;
    }

    const xpCurRemaining =
      mode === 'xp' ? xpExact : xpLvl(levelFrom) * (pct / 100);

    if (xpCurRemaining <= 0) {
      setError('Informe o XP restante.');
      return;
    }
    if (xpCurRemaining > xpLvl(levelFrom)) {
      setError(
        `XP restante não pode ser maior que o total do level (${xpLvl(levelFrom).toLocaleString('pt-BR')}).`
      );
      return;
    }
    ...
    function timeForXP(xp: number, rate: number) {
      ...
      if (hasStam && stamConsumed < STAM_H) {
        const stamAvail = STAM_H - stamConsumed;
        const multStam = getMultiplier(rate, true, hasStore);
        const xphStam = rawPH * multStam;
        const maxXpInStam = xphStam * stamAvail;

        if (remaining <= maxXpInStam) {
          const h = remaining / xphStam;
          stamConsumed += h;
          hours += h;
          return hours;
        } else {
          hours += stamAvail;
          stamConsumed = STAM_H;
          remaining -= maxXpInStam;
        }
      }

      const multNoStam = getMultiplier(rate, false, hasStore);
      hours += remaining / (rawPH * multNoStam);
      return hours;
    }
    ...
    totalH += hCur;
    breakdown.push({
      label: `Lv ${levelFrom} (${pctLabel} · ${rateCur}x)`,
      xp: xpCurRemaining,
      h: hCur,
    });
          ...
          <div className="mb-5 grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setHSess(Number(e.target.value))}
                className="h-11 w-full rounded-2xl border border-white/10 bg-[#0b1220] px-4 text-sm text-white outline-none focus:border-violet-500/50"
              />
            </div>
          </div>
              ...
              <div>
                ...
                <div className="flex flex-col gap-1">
                  {result.breakdown.map((s, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between rounded-xl border border-white/5 bg-white/5 px-4 py-2.5 text-sm"
                    >
